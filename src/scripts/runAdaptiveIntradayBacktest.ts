...
try {
  ...
  const engine =
    new MultiSymbolIntradayBacktestEngine();
  ...
  console.log("");
  console.log(
    "=== Adaptive Intraday Strategy Backtest ==="
  );
  console.log(
  ...
  console.log(
    `Execution:           5m SignalEvaluator + ATR stop/target`
  );
  console.log(
    `Intraday symbols:    ${result.intradaySymbols.join(", ")}`
  );
  console.log(
  ...
  console.log(
    `Signal exits:        ${result.signalExits}`
  );
  console.log(
    `End-of-day exits:    ${result.endOfDayExits}`
  );
  console.log(
  ...
  for (const selection of result.selections.slice(-8)) {
    console.log(
      `${selection.date}  ${selection.symbols.join(", ") || "(no 5m data)"}`
    );
  }

  if (result.intradaySymbols.length < 5) {
    console.log("");
    console.log(
      "Note: Only symbols with 5m data in SQLite are traded intraday. " +
      "Backfill 5m bars to expand intraday coverage."
    );
  }
} finally {
  repository.close();
